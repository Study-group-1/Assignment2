...
#Mean-centering the Altercentric Intrusion variable 
d <- d %>% mutate(
...
#Make a model (both null and model with predictors)
ai_F0 <- brms::bf(ai ~ 1)
...
#Make priors for intercept, sigma, and intercept
ai_p0 <- c(
...
ai_p1 <- c(
  brms::prior(normal(0, 1), class = Intercept),
  brms::prior(normal(1, 0.5), class = sigma),
  brms::prior(normal(0, 0.2), class = b, coef = di )
)
...
ggplot(post) +
  ...
  geom_density(aes(b_di), fill="blue", alpha=0.5) 
